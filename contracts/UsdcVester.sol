...
contract UsdcVester is IVester, IERC20, ReentrancyGuard, Ownable {
    ...
    function withdraw() external nonReentrant {
        ...
        if (hasPairToken()) {
            ...
            IERC20(pairToken).safeTransfer(
                _receiver,
                pairAmount / 10 ** (decimals - usdcDecimals)
            );
        }

        IERC20(fidToken).safeTransfer(_receiver, balance);
    ...
    function getCombinedAverageStakedAmount(
        ...
        uint256 convertedAverageStakedAmount = averageStakedAmount.mul(
            10 ** (decimals - usdcDecimals)
        );
    ...
    function _deposit(address _account, uint256 _amount) private {
        ...
        IERC20(fidToken).safeTransferFrom(_account, address(this), _amount);
        ...
        if (hasPairToken()) {
            ...
            if (nextPairAmount > pairAmount) {
                ...
                IERC20(pairToken).safeTransferFrom(
                    _account,
                    address(this),
                    pairAmountDiff / 10 ** (decimals - usdcDecimals)
                );
    ...
    function _updateVesting(address _account) private {
        ...
        IMintable(fidToken).burn(address(this), amount);
    }
